...
export async function generateWithTrainedModel(
  ...
  // NSFW e Super Realism auto-inject desligados por padrao. Usuario pode ligar explicitamente.
  const allLoras: LoraInput[] = [
    ...(input.nsfwEnabled === true ? [{ url: NSFW_LORA, scale: input.nsfwScale ?? NSFW_LORA_SCALE }] : []),
    ...(input.realismEnabled === true ? [{ url: SUPER_REALISM_LORA, scale: input.realismScale ?? SUPER_REALISM_LORA_SCALE }] : []),
    ...input.loras,
  ];
