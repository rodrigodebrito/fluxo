...
export default function UGCCampaign({ onBack }: Props) {
  ...
  // Extrai o campo "value" de uma resposta do GPT no formato {"value": "..."}.
  // Aceita: code fence, prose antes/depois, JSON direto, ou fallback pra string crua
  // se o modelo ignorar o schema. Retorna "" se nao conseguir extrair nada util.
  function extractFieldValue(raw: unknown): string {
    ...
    // Fallback: alguns modelos ignoram o schema e voltam so o texto.
    // Se nao tem chaves, assume que o texto inteiro e o valor (stripando aspas).
  ...
  const handleRegenerateField = async (sceneNumber: number, field: SceneField) => {
    ...
    // Snapshot pra restaurar caso o parse falhe. So empurramos pro undo stack
    // DEPOIS que o novo estado foi aplicado com sucesso, pra nao poluir o stack
    // com transicoes que nunca aconteceram.
    const snapshotBefore = campaign;
    ...
    try {
      ...
      // Parse defensivo: extrai {value: "..."} mesmo se GPT voltar
      // code fence, prose ao redor, trailing comma, ou string crua.
      const newValue = extractFieldValue(data.text);
      if (!newValue && field !== "spokenLine") {
        // voiceMode=none pode legitimamente retornar "" pra spokenLine, os outros nao
      ...
      setCampaign({
        ...snapshotBefore,
        scenes: snapshotBefore.scenes.map((s) => {
